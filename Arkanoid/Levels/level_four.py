...
def lvl_four() -> None:
    ...
    paddle, bouncer, FONT = set_up(75, 35, 60, 5, bricks_hp, bricks_colors)
    ...
    while gl.PLAY:
            ...
            if gl.LIVES == 0:
                ...
                while gl.PAUSE:
                    ...
                    if event.type == gl.KEYDOWN_EVENT and (event.key == gl.R_KEY):
                        ...
                        gl.SPRITES.empty()
                        gl.BRICKS.empty()
                        paddle, bouncer, FONT = set_up(75, 35, 60, 3, bricks_hp, bricks_colors)
                    elif event.type == gl.CLOSE_WINDOW:
                        gl.PAUSE = False
                        gl.PLAY = False
                    elif event.type == gl.KEYDOWN_EVENT and (event.key == gl.ESC_KEY or event.key == gl.Q_KEY):
        ...
        for b in brick_collisions:
            ...
            if len(gl.BRICKS) == 0:
                ...
                text = font.render("Final level starts in 3 seconds", 1, gl.WHITE)
                gl.SCREEN.blit(text, (int((gl.WIDTH - 300) / 2), 360))
                pygame.display.flip()
                gl.PLAY = False
                gl.LVL4_COMPLETE = True
